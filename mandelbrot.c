...
unsigned char mandelbrot_buffer[OUTPUT_WIDTH * 3 * OUTPUT_HEIGHT];

void mandelbrot(int coreId, int coreCount) {
    unsigned int y_slice = OUTPUT_HEIGHT / coreCount; /* Compute slice size for multi-core bench */
    unsigned int y_beg = coreId * y_slice;
    unsigned int y_end = y_beg + y_slice;
    ...
    for (y = y_beg; y < y_end; ++y) {
        ...
        for (x = 0; x < OUTPUT_WIDTH; ++x) {
            ...
            mandelbrot_buffer[3 * x * y] = ((color >> 11) & 0x1F) << 3;
            mandelbrot_buffer[3 * x * y + 1] = ((color >> 5) & 0x3F) << 2;
            mandelbrot_buffer[3 * x * y + 2] = (color & 0x1F) << 3;
        }
    }
